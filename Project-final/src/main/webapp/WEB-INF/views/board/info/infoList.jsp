			...
			<h3>
				...
				<table class="table table-hover">
						...
						<c:choose>
							...
							<c:otherwise>
								<c:forEach var="item" items="${list }">
									<tr
										onclick="location.href='/info/detail.do?boardIdx=${item.idx}'">
										<td>${row}</td>
										<td>${item.category}</td>
										<td>${item.title}</td>
										<td>${item.writer}</td>
					...
					<div class="d-grid gap-2d-md-flex justify-content-end float-right ">
						<button class="btn btn-dark me-md-2" onclick="window.location.href='/info/enrollForm.do'" type="button">글쓰기</button>
					</div>
				</table>
