	...
	<%@ include file="/WEB-INF/views/common/header.jsp" %>
		...
		<br/>
			  ...
			  <c:choose>
			  	...
			  	<c:otherwise>
			  		<c:forEach var="item" items="${list }">
			  			<tr onclick="location.href='/free/detail.do?boardIdx=${item.idx}'">               
			  				<td>${row}</td>
			  				<td>${item.title}</td>
			  				<td>${item.writer}</td>
			  				<td>${item.indate}</td>
	      ...
	      <div class="d-grid gap-2d-md-flex justify-content-end float-right ">
				  <button class="btn btn-dark me-md-2" onclick="window.location.href='/free/enrollForm.do'" type="button" >글쓰기</button>
				</div> 
				</table>
